...
//     return princeToken.deployed().then(function(instance) {
//       tokenInstance = instance;
//       return tokenInstance.name();
//     }).then(function(name) {
//       assert.equal(name, 'prince Token', 'has the correct name');
//       return tokenInstance.symbol();
//     }).then(function(symbol) {
//       assert.equal(symbol, 'prince', 'has the correct symbol');
//       return tokenInstance.standard();
//     }).then(function(standard) {
//       assert.equal(standard, 'prince Token v1.0', 'has the correct standard');
//     });
//   })
...
contract("princeToken", function (accounts) {
  ...
  it("initializes the contract with the correct values", function () {
    return princeToken
      ...
      .then(function (name) {
        assert.equal(name, "prince Token", "has correct name");
        return tokenInstance.symbol();
      })
      .then(function (symbol) {
        assert.equal(symbol, "prince", "has correct symbol");
        return tokenInstance.standard();
      })
      .then(function (standard) {
        assert.equal(standard, "prince Token v1.0", "Has correct standard");
      });
  });
  ...
  it("handles delegated token transfers", function () {
    return princeToken
      ...
      .catch(function (error) {
        assert(
          error.message.indexOf("revert") >= 0,
          "cannot transfer valuer larger than approved amount"
        );
